...
c <- c |>
  mutate(proposed_sampling_org = case_when(
    proposed_sampling_org %in% c("WLRS", "MOTT or WLRS", "MOTT") ~ "BC Gov (WLRS/MOTT)",
...
d_with_geometries = d_with_geometries |> 
  dplyr::left_join(c_f |> 
                     dplyr::select(
                       `Waterbody Name`, 
                       `Watershed Name`= `Sub-Watershed/Reach Name`,
                       proposed_site_name = `Proposed Sample site name`,
                       proposed_sampling_org = `Proposed sampling organization`)
  )
...
# display.brewer.all()
org_pal = leaflet::colorFactor(domain = orgs, palette = 'Dark2')
...
leaflet() |> 
  ...
  addCircleMarkers(
    data = sample_points_sf,
    color = 'black',
    fillColor = 'black',
    radius = 3,
    label = ~paste0(`Proposed sampling organization`,", ",`Waterbody Name`),
    popup = leafpop::popupTable(sample_points_sf,
                       zcol = c("Waterbody Name",
                                "Sub-Watershed/Reach Name",
                                "Final sample site name",
                                "Proposed sampling organization",
                                "Fish species to be sampled",
