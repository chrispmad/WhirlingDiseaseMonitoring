...
wb_list = wb_list |> 
  ...
  dplyr::select(GNIS_NA:days_fished_bin,
                ...
                priority,
                fish_edna_positive_in_watershed:infectable,tubifex_edna_positive_in_watershed:SARA)
```
...
wb_list_SARAver_3 <- wb_list |> filter(priority > 3)

priority_vals <- sort(unique(wb_list_SARAver_3$priority))
...
for (p in priority_vals) {
  wb_subset <- wb_list_SARAver_3 |> filter(priority == p)
  ...
  m <- m |> addPolygons(
    ...
    popup = leafpop::popupTable(
      wb_subset |> st_drop_geometry() |> 
        select(GNIS_NA, susceptible_spp, sara, stocked_species, known_fish, TotalInspections_kmeans_bin, insp_from_wd_bc_wb_kmeans_bin, days_fished_kmeans_bin, priority)
    )
  )
}
