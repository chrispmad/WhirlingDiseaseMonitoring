...
water_apps_w_wb_info = water_apps |> 
  dplyr::filter(approval_status %in% c('Current','Expired')) |>
  sf::st_join(wb_list |> dplyr::select(GNIS_NA,WATERSH))

water_apps_w_wb_cumulative = water_apps |> 
  dplyr::filter(approval_status %in% c("Current","Superseded",'Expired')) |>
  sf::st_join(wb_list |> dplyr::select(GNIS_NA,WATERSH)) |> 
  sf::st_drop_geometry() |> 
  dplyr::filter(!is.na(GNIS_NA)) |> 
...
number_water_apps_per_wb = water_apps_w_wb_info |> 
  dplyr::filter(!is.na(GNIS_NA)) |> 
  sf::st_drop_geometry() |> 
  dplyr::count(GNIS_NA,WATERSH, name = "active_and_expired_water_authorizations")


# Bin!
number_water_apps_per_wb = bin_to_kmeans(number_water_apps_per_wb, 'active_and_expired_water_authorizations', k_breaks)

water_auths_bins = getKMeansBreaks(number_water_apps_per_wb$active_and_expired_water_authorizations, k = k_breaks)
...
if(!interactive()){
 ggplot() + 
  geom_sf(data = bcmaps::bc_bound()) +
  geom_sf(data = wbs_with_water_apps, aes(col = active_and_expired_water_authorizations_kmeans_bin, fill = active_and_expired_water_authorizations_kmeans_bin)) +
  scale_fill_brewer(palette = 'Spectral', direction = -1) +
  scale_color_brewer(palette = 'Spectral', direction = -1) +
  ggthemes::theme_map() +
...
wb_list = wb_list |> 
  ...
  dplyr::mutate(active_and_expired_water_authorizations = tidyr::replace_na(active_and_expired_water_authorizations, 0))
```
...
if(requery_occs){
  species_occs <- lapply(species_names, function(sp) {
    ocs<-try(suppressMessages(suppressWarnings(grab_aq_occ_data(sp, quiet = TRUE))))
    if (!is.null(ocs)) {
...
sara_sp<-sf::read_sf(paste0(onedrive_wd,"DFO_SARA/dfo_sara_occurrences_in_BC_no_marine.gpkg"))
...
$$
P_w = IEBC_w + IIBC_w + DF_w + WA_w
$$

Where *P~w~* is the priority ranking of a given waterbody, IEBC~w~ is the binned number of watercraft destined for a given waterbody from a Whirling Disease-infected state/province/territory that is not BC, IIBC is the binned number of watercraft destined for a given waterbody coming from a Whirling Disease-infected waterbody in BC, DF~w~ is the binned fishing days reported for a given waterbody, and WA~w~ is the binned number of current or expired water authorization points within a given waterbody.
...
wb_list <- wb_list |> filter(priority > 3)

priority_vals <- sort(unique(wb_list$priority))
...
for (p in priority_vals) {
  wb_subset <- wb_list |> filter(priority == p)
  ...
  m <- m |> addPolygons(
    ...
    popup = leafpop::popupTable(
      wb_subset |> st_drop_geometry() |> 
        select(GNIS_NA, Fish_present, sara, TotalInspections_kmeans_bin, insp_from_wd_bc_wb_kmeans_bin, days_fished_kmeans_bin, priority)
    )
  )
}
