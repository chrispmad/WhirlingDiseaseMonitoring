...
saveRDS(wb_list, paste0(onedrive_wd,"output_of_WD_monitoring_analysis_waterbodies.rds"))
```
...
wb_list_final <- bind_rows(indig_wbs, remaining_wbs |> dplyr::mutate(Flagged_by_FN_partners = NA)) |> 
  arrange(desc(Flagged_by_FN_partners), desc(priority))
...
make_final_column_names = function(dat){
  dat = dat |> 
    ...
    dplyr::rename(`Waterbody Name` = GNIS_NA, 
                ...
                `Opportunisitic sampling (please insert name)` = opportunistic_sampling
                )
  if("BLK" %in% names(dat)){
    ...
    dplyr::rename(`Boats entering BC (bins 1 - 5)` = boats_entering_BC_bin,
      ...
      dplyr::select(`Waterbody Name`, `Watershed Code`, `Watershed Name`,
                    Priority = priority, `Identified by FN partners`, dplyr::everything())
}
...
openxlsx::writeData(my_wb, "Top 100 priority list", 
...
openxlsx::setColWidths(my_wb, "Top 100 priority list", cols = c(1:(ncol(wb_list_final_for_excel)-1)), widths = 'auto')
...
file.copy(from = paste0("output/Whirling_Disease_priorities_for_waterbody_sampling_",Sys.Date(),".xlsx"),
          to = paste0("J:/2 SCIENCE - Invasives/SPECIES/Whirling Disease/Monitoring/2025/Whirling_Disease_priorities_for_waterbody_sampling_",Sys.Date(),".xlsx"), overwrite = T)
```
