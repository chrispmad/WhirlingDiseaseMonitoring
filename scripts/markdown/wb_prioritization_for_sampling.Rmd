...
tubifex_maxent<- terra::rast("output/habitat_maxent_tubifex_fc.LQ_rm.4.tif")
...
bc_crs <- "EPSG:3005"
# terra::crs(tubifex_maxent) = "EPSG:4326"
...
extracted <- terra::extract(tubifex_maxent, wb_vect)
# extracted <- as.data.frame(extracted)
# factor_levels$fc.LQ_rm.3 <- as.numeric(factor_levels$fc.LQ_rm.3)
extracted <- dplyr::left_join(extracted, factor_levels, by = "fc.LQ_rm.4")

suitability_summary <- extracted |>
  group_by(ID.x) |>
  summarize(
    max_val = if (all(is.na(fc.LQ_rm.4))) NA_real_ else max(as.numeric(fc.LQ_rm.4), na.rm = TRUE)
  ) |>
  mutate(
...
maxent_rast<-terra::rast("output/habitat_maxent_tubifex_fc.LQ_rm.4.tif")

if(!interactive()){
ggplot() +
  tidyterra::geom_spatraster(data = maxent_rast, aes(fill = fc.LQ_rm.4))+
