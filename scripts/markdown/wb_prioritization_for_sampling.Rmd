...
water_apps_w_wb_cumulative = water_apps |> 
  dplyr::filter(approval_status == "Current" | approval_status == "Superseded")|>
  sf::st_join(wb_list |> dplyr::select(GNIS_NA,WATERSH)) |> 
  sf::st_drop_geometry() |> 
  dplyr::filter(!is.na(GNIS_NA)) |> 
...
if(requery_occs){
  species_occs <- lapply(species_names, function(sp) {
    ocs<-suppressMessages(suppressWarnings(grab_aq_occ_data(sp, quiet = TRUE)))
    if (!is.null(ocs)) {
