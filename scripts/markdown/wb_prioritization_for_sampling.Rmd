...
suitability_summary <- extracted |> 
  group_by(ID) |> 
  # if there is a single cell with a 1, then the whole waterbody would be suitable
  summarize(max_val = max(as.numeric(fc.LQ_rm.3), na.rm = TRUE)) |> 
  mutate(tubifex_habitat_suitability = ifelse(max_val >= 0.5, "suitable", "unsuitable"))
