...
new_wb_for_list = insp_from_wd_bc_wb_b |> 
  ...
  dplyr::select(GNIS_NA, WATERSH,
                ...
                insp_from_wd_bc_wb_b = TotalInspections_kmeans_bin )
...
leaflet() |> 
  ...
  addLayersControl(baseGroups = c("cartoDB","openStreetMap"),
                   overlayGroups = c("adjacent","infectable","infected",'infectable wbs','PSCIS','dams','positive fish samples'),
                   options = layersControlOptions(collapsed = F)) |> 
  addMapPane("wsg_p", zIndex = 200) |> 
  addMapPane("adjacent_p", zIndex = 210) |> 
  addMapPane("infectable_p", zIndex = 300) |> 
  addMapPane("infected_p", zIndex = 400) |> 
  addMapPane("infectable_wbs_p", zIndex = 500) |> 
  addMapPane("PSCIS_and_dams_p", zIndex = 600) |> 
  addMapPane("pos_fish_p", zIndex = 700) |> 
...
wb_list = wb_list |> 
  dplyr::mutate(priority = as.numeric(TotalInspections_kmeans_bin, na.rm = T) + as.numeric(insp_from_wd_bc_wb_b, na.rm = T) + as.numeric(days_fished_kmeans_bin, na.rm = T))
```
