...
if(!file.exists('data/anger_survey.rds')){
  ...
  for(i in 1:nrow(insp_from_wd_inf_to_wb)){
    ...
    if(is.na(the_wb$BLK)){
      ...
      if(str_detect(the_wb$GNIS_NA,"River")){
...
# note, I changed this from the binary output and now things seem to not be working well... gah!
...
```{r}
### Water Quality: DO and Phosphorous

# We queried the Environmental Monitoring System database for records mentioning the keywords "mine" or "mill", and for parameters Dissolved Oxygen and Phosphorous, for a first pass at water quality.
# 
# ![](images/EMS_mine_mills_dissolved_oxygen_data.jpg)
# 
# ![](images/EMS_mine_mills_phosphorous_data.jpg)
```
...
# rename the columns, first is ID, then the months
# month_names <- month.abb[1:(ncol(monthly_temps_extracted) - 1)]
# new_names <- c("ID", paste0(month_names, "_temp"))
# names(monthly_temps_extracted) <- new_names
...
if (nrow(new_streams) > 0) {
  new_streams <- new_streams |>
    st_transform(4326) |>
    group_by(GNIS_NAME, WATERSHED_GROUP_ID) |>
    summarise()
} else {
  new_streams <- st_sf(
    GNIS_NAME = character(),
...
nations_wb<-bind_rows(new_wbs, new_streams, new_lakes) |> 
  select(GNIS_NAME_1, WATERSHED_GROUP_ID, geometry) 
...
wb_list <- wb_list |> 
  dplyr::bind_rows(
    nations_priorities |> 
      ...
      dplyr::rename(GNIS_NA = GNIS_NAME_1, WATERSH = WATERSHED_GROUP_ID)
  )
...
l = leaflet() |> 
  ...
  addLayersControl(
    position = 'bottomleft',
    overlayGroups = c(years_sampled, "not sampled 2024", "SARA present", "SARA absent", "Indigenous priority"),
    options = layersControlOptions(collapsed = FALSE)
  )
...
l |> 
  ...
  addLegend(position = "bottomright", title = "Waterbodies",
            ...
            labels = c("SARA present", "SARA absent"),
            opacity = 1)
