...
### Watersheds Downstream of Fish Samples with WD
```{r}
library(bcdata)
...
# Grab rivers in infected little watersheds. This is a first pass,
# we may need to requery the BC data catalogue with the 
# identifying codes (e.g. Blue line key, or fwa watershed code)
# of the river chunks returned.
...
# blks_of_infected_rivers = unique(infected_rivers$BLUE_LINE_KEY)
# 
# infected_rivers = bcdc_query_geodata('freshwater-atlas-rivers') |> 
#   filter(BLUE_LINE_KEY %in% blks_of_infected_rivers) |> 
...
ggplot() + 
  ...
  geom_sf(data = adj_small_watersheds, fill = 'cyan') + 
  geom_sf(data = infectable_adj_watersheds, fill = 'orange') + 
  geom_sf(data = small_watersheds_infected, fill = 'red') + 
  geom_sf(data = rivers_in_infectable_adj_watersheds, fill = 'purple', col = 'purple') +
  geom_sf(data = lakes_in_infectable_adj_watersheds, fill = 'purple', col = 'purple') +
  geom_sf(data = infected_rivers, fill = 'darkblue', col = 'darkblue') + 
  geom_sf(data = pscis_in_area, col = 'green', alpha = 0.1) +
...
wb_list = wb_list |> 
  dplyr::mutate(priority = as.numeric(TotalInspections_bin, na.rm = T) + as.numeric(insp_from_wd_bc_wb_b, na.rm = T) + as.numeric(days_fished_bin, na.rm = T))
```
