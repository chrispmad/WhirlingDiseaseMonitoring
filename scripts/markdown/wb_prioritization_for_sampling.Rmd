...
if(length(tub_files) > 0){
  ...
  wb_list$tubifex_habitat_suitability <- suitability_summary$tubifex_habitat_suitability
    ...
    # hab_not_hab_tub_maxent$hab.not.hab = as.character(hab_not_hab_tub_maxent$hab.not.hab)
...
```{r previous_wd_sampling_and_2024_identified_unsampled_sites, fig.width = 9, fig.height = 6}
...
### Whirling Disease Sampling with Waterbodies with SARA overlaps
...
l = leaflet() |> addTiles() |> addLayersControl(
  position = 'bottomleft',
  overlayGroups = years_sampled,
  options = layersControlOptions(collapsed = FALSE)
)
...
wb_over_5_in_frascol = wb_list[wb_list$priority >= 5,] |> 
