...
make_final_column_names = function(dat){
  dat = dat |> 
    ...
    dplyr::rename(`Waterbody Name` = GNIS_NA, 
                ...
                `Potential opportunisitic sampling` = opportunistic_sampling
                )
  if("BLK" %in% names(dat)){
...
# Final re-ordering of columns for final tab.
wb_list_final_nice_names = wb_list_final_nice_names |> 
  dplyr::select(`Waterbody Name`, `Watershed Code`, `Watershed Name`,
                Priority,sampled_2024, `Identified by FN partners`,
                `Potential opportunisitic sampling`,SARA,
                dplyr::everything()) |> 
