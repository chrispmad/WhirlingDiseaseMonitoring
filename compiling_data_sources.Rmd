...
We searched the BC Data Catalogue for hatcheries or 'salmonid enhancement' keywords: no results!

Canada Open Data (hosted federally) has a data source that describes fish health; of the 7,056 rows in this table, 5,357 rows describe some fish health issue at a location that includes the key word "HATCHERY".
```{r}
# Nothing in BC data catalogue.

# Canada Open Data portal offers a fish health dataset that describes (some? all?) fisheries in BC.
# URL: https://open.canada.ca/data/en/dataset/2ece9991-62aa-4b7a-bd7d-4f8f1052cd21
hatcheries = readr::read_csv("data/Fish_Health_Database.csv") |> 
  # Replace column names
  purrr::set_names(nm = c("case","start_date","location_name","lat","long","species","life_stage","etiological")) |> 
  # Convert lat and lng to numeric
  dplyr::mutate(lat = as.numeric(lat), long = as.numeric(long)) |> 
  dplyr::filter(!is.na(lat), !is.na(long)) |>
  # Drop first row, which was just a double column name (English and French)
  dplyr::filter(location_name != "NOM DU LIEU")

# Filter for just those rows where we find 'HATCHERY' in the location name field.
hatcheries = hatcheries |> 
  dplyr::filter(stringr::str_detect(location_name,"HATCHERY")) |> 
  # Keep just one row per hatchery.
  dplyr::select(start_date,location_name,species,lat,long) |> 
  # Find earliest date in table for each location
  dplyr::group_by(location_name) |> 
  dplyr::mutate(earliest_date = min(start_date),
...
leaflet() |> 
  ...
  addCircleMarkers(
    data = hatcheries,
    label = ~location_name,
    popup = lapply(leafpop::popupTable(
      hatcheries |> sf::st_drop_geometry()
      ),htmltools::HTML)
  )
```

```{r}
DT::datatable(hatcheries |> sf::st_drop_geometry())
```
